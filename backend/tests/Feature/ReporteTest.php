...
class ReporteTest extends TestCase
{
    ...
    /**
     * 4.11 — test_rendimiento_nivel_agencia_agrupa_por_locales:
     * El reporte de rendimiento con nivel=agencia agrupa por LOCAL y
     * etiqueta la fila con la clave 'Agencia' (nombre del local).
     */
    public function test_rendimiento_nivel_agencia_agrupa_por_locales()
    {
        $super = User::where('email', 'super@lotto.com')->first();
        $super->assignRole('super_master');
        ...
        $banca = Banca::create(['name' => 'Banca Rend Locales', 'code' => 'BRDL', 'active' => true]);
        $grupo = Grupo::create(['name' => 'Grupo Rend Locales', 'code' => 'GRDL', 'banca_id' => $banca->id, 'active' => true]);

        $localUno = Agencia::factory()->create(['name' => 'Local Rend Uno', 'grupo_id' => $grupo->id, 'active' => true]);
        $localDos = Agencia::factory()->create(['name' => 'Local Rend Dos', 'grupo_id' => $grupo->id, 'active' => true]);

        $taquillaUno = Taquilla::create(['name' => 'T-Rend-1', 'code' => 'TRD1', 'grupo_id' => $grupo->id, 'agencia_id' => $localUno->id, 'active' => true]);
        $taquillaDos = Taquilla::create(['name' => 'T-Rend-2', 'code' => 'TRD2', 'grupo_id' => $grupo->id, 'agencia_id' => $localDos->id, 'active' => true]);
        ...
        $this->assertCount(2, $data, 'Debe haber 2 filas, una por local');
        $porLocal = collect($data)->keyBy('Agencia');
        $this->assertTrue($porLocal->has('Local Rend Uno') && $porLocal->has('Local Rend Dos'), 'Ambos locales deben estar presentes');
        $this->assertEquals(1000, $porLocal['Local Rend Uno']['Venta']);
        $this->assertEquals(500, $porLocal['Local Rend Dos']['Venta']);
    }

    /**
     * 4.12 — test_rendimiento_nivel_taquilla_usa_clave_taquilla:
     * Con nivel=taquilla (default) el rendimiento agrupa por máquina y
     * etiqueta la fila con la clave 'Taquilla' (label máquina).
     */
    public function test_rendimiento_nivel_taquilla_usa_clave_taquilla()
    {
        ...
        $banca = Banca::create(['name' => 'Banca Rend Maquinas', 'code' => 'BRDM', 'active' => true]);
        $grupo = Grupo::create(['name' => 'Grupo Rend Maquinas', 'code' => 'GRDM', 'banca_id' => $banca->id, 'active' => true]);
        $local = Agencia::factory()->create(['name' => 'Local Rend Maquinas', 'grupo_id' => $grupo->id, 'active' => true]);

        $taquillaUno = Taquilla::create(['name' => 'T-RendM-1', 'code' => 'TRM1', 'grupo_id' => $grupo->id, 'agencia_id' => $local->id, 'active' => true]);
        $taquillaDos = Taquilla::create(['name' => 'T-RendM-2', 'code' => 'TRM2', 'grupo_id' => $grupo->id, 'agencia_id' => $local->id, 'active' => true]);
        ...
        // Sin parámetro nivel → default taquilla
        $response = $this->actingAs($super, 'sanctum')
            ->getJson('/api/v1/reportes/rendimiento-taquillas');
        ...
        $this->assertCount(2, $data, 'Debe haber 2 filas, una por máquina');
        $porMaquina = collect($data)->keyBy('Taquilla');
        $this->assertTrue($porMaquina->has('T-RendM-1') && $porMaquina->has('T-RendM-2'), 'Ambas máquinas deben estar presentes');
        $this->assertEquals(300, $porMaquina['T-RendM-1']['Venta']);
        $this->assertEquals(200, $porMaquina['T-RendM-2']['Venta']);
    }
}
