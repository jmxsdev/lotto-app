...
use Database\Seeders\LaGranjitaSeeder;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Tests\TestCase;

class LaGranjitaResultsTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected function setUp(): void
    {
        ...
        $this->seed(LaGranjitaSeeder::class);
    }
    ...
    public function test_seeder_registra_juego_con_scraper_class(): void
    {
        ...
        $this->assertEquals('La Granjita', $juego->name);
        $this->assertEquals('animalitos', $juego->type);
        $this->assertTrue($juego->requires_scraper);
        $this->assertEquals('https://www.lagranjita.com/api/results.json?productId=1', $juego->scraper_url);
        $this->assertEquals(LaGranjitaScraper::class, $juego->scraper_class);
    ...
    public function test_seed_y_scrape_persisten_resultados(): void
    {
        ...
        $juego = Juego::where('slug', 'la-granjita')->first();
        $scraper = new LaGranjitaScraper($juego);
        $json = file_get_contents(base_path('tests/Fixtures/lagranjita_results.json'));
        ...
        $this->assertEquals(12, $persistido->numeros_ganadores['numero']);
        $this->assertEquals('CABALLO', $persistido->numeros_ganadores['nombre_animal']);
        $this->assertEquals('VE', $persistido->numeros_ganadores['pais']);
        $this->assertEquals('414469', $persistido->sorteo_id_externo);
    }
    ...
    public function test_resolve_scraper_usa_scraper_class_registrado(): void
    {
        ...
        $this->assertEquals(LaGranjitaScraper::class, $method->invoke($job, $juego));
    }
}
