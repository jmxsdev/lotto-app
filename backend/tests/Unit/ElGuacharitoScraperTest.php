...
class ElGuacharitoScraperTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $html = file_get_contents(base_path('tests/Fixtures/loteriadehoy_elguacharito.html'));

        $resultados = $this->invocar('parse', $html);

        $this->assertIsArray($resultados);
        // La página hoy solo muestra los sorteos ya ocurridos (08:30, 09:30, 10:30 y 11:30),
    ...
    public function test_parsea_numero_y_animal_por_bloque(): void
    {
        ...
        $this->assertEquals([95, 33, 11, 34], $numeros);

        $animales = array_column(array_column($resultados, 'numeros_ganadores'), 'nombre_animal');
        $this->assertEquals(['Escarabajo', 'Pescado', 'Gato', 'Venado'], $animales);
    }
    ...
    public function test_ignora_bloques_malformados_sin_hora_ni_numero(): void
    {
        $html = '<html><body><div class="row text-center js-con">'
            ...
            .'<div class="circle-legend"><h4 class="mt-3 rojo">95 Escarabajo</h4><h5>El Guacharito Millonario 08:30 AM</h5></div>'
            .'</div>'
            .'<div class="col-sm-6 col-md-4 col-lg-16 mb-5">'
            .'<div class="circle-legend"><h4 class="mt-3 rojo">-- Sin dato</h4><h5>El Guacharito Millonario 09:30 AM</h5></div>'
            .'</div>'
            .'</div></body></html>';
        ...
        $this->assertEquals('08:30', $resultados[0]['hora_sorteo']);
        $this->assertEquals(95, $resultados[0]['numeros_ganadores']['numero']);
    }
}
