...
| H2 | Zoológicos mayores al canónico: **monje 77, guácharo 77, guacharito 101** (números > 36 reales en resultados). | **Monje Millonario CONFIRMADO con fuente oficial y CORREGIDO (WU f22)**: el feed oficial de lottoactivo.com muestra números 0–74 con zoo propio (49=Pereza, 42=Tucán, 74=Turpial…). Se crearon **70 figuras confirmadas** (patrón Loto Chaima) y quedan **7 números sin nombre oficial** (37/39/57/65/67/68/75; el 75 sería El Patronus). Pendiente aún: `guácharo-activo` (77) y `el-guacharito` (101) — su fuente actual es el agregador, falta verificación oficial. Afecta plugin Animalitos (validación 0–36 → con tabla propia la validación usa los labels de `juego_opciones`). |
